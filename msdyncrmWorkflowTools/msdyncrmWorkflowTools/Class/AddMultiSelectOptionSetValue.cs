...
{
    public class AddMultiSelectOptionSetValue : CodeActivity
    {
        [RequiredArgument]
        [Input("Values")]
        ...
        protected override void Execute(CodeActivityContext executionContext)
        {
            ...
            try
            {
                ...
                newEntity.Attributes[attributeName] = AddValuesInOptionSetValueCollection(values, optionSetValueCollection);
        ...
        private OptionSetValueCollection AddValuesInOptionSetValueCollection(string[] values, OptionSetValueCollection optionSetValueCollection)
        {
            if (optionSetValueCollection == null) optionSetValueCollection = new OptionSetValueCollection();

            foreach (var value in values)
            {
                if (!optionSetValueCollection.Any(x => x.Value.ToString() == value))
                {
                    optionSetValueCollection.Add(new OptionSetValue(int.Parse(value)));
                }
            }
